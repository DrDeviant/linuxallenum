...
while true; do
	...
	echo -ne " 137. sensepost/DNS-Shell\n"
	echo "UPNP"
	...
	"10")
		...
		select EXP in $(lynx -dump -listonly "$ENTSSL""offensive-security/exploitdb/tree/master/exploits/linux/local" | grep -E "\.sh$|\.py$|\.c$|\.pl$|\.rb$|\.asm$|\.txt$|\.java$|\.php$" | awk '{print $2}' | while read -r EXP; do echo "${EXP/$ENTFRM/}"; done)
		do
			wget "$ENTTO""$EXP"
			break
		done
	...
	"11")
		...
		echo "(example exploit.py)"
		select EXP in $(lynx -dump -listonly "$ENTSSL""offensive-security/exploitdb/tree/master/exploits/linux_x86-64/local"| grep -E "\.sh$|\.py$|\.c$|\.pl$|\.rb$|\.asm$|\.txt$|\.java$|\.php$" | awk '{print $2}' | while read -r EXP; do echo "${EXP/$ENTFRM/}"; done)
		do
			wget "$ENTTO""$EXP"
			break
		done
	...
	"12")
		...
		echo "(example exploit.py)"
		select EXP in $(lynx -dump -listonly "$ENTSSL""offensive-security/exploitdb/tree/master/exploits/linux_x86/local"| grep -E "\.sh$|\.py$|\.c$|\.pl$|\.rb$|\.asm$|\.txt$|\.java$|\.php$" | awk '{print $2}' | while read -r EXP; do echo "${EXP/$ENTFRM/}"; done)
		do
			wget "$ENTTO""$EXP"
			break
		done
